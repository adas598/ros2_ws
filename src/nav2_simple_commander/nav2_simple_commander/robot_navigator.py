...
class BasicNavigator(Node):
    ...
    def __init__(self):
        ...
        self.interpolation_resolution = 0.1
    ...
    def goStraightToPose(self, pose):
        ...
        iter_pose = PoseStamped()

        for i in len(total_no_loops):
            
            iter_pose.pose.position.x = self.initial_pose.pose.position.x + x_increment * i
            iter_pose.pose.position.y = self.initial_pose.pose.position.y + y_increment * i
            iter_pose.pose.position.z = 0.0
            iter_pose.pose.orientation = pose.pose.orientation  # FIXME: Verify if this is ok
            iter_pose.header.stamp = self.get_clock().now().to_msg()
            iter_pose.header.frame_id = "map"
            points.append(iter_pose)

        # End of computation of straight line points 

        for waypoint in points:
            goal_msg = NavigateToPose.Goal()
            goal_msg.pose = waypoint   # FIXME: 'waypoint' is an iteration variable which holds the interpolated points
